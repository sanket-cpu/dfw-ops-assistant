...
llm = ChatOpenAI(model="gpt-4.1-mini", temperature=0,max_completion_tokens=800)
...
METADATA_CONFIG = {
    "DFW-Airports.pdf": {
        "airport": "dfw",
        "doc_type": "official",
        "source_name": "DFW-Airports",
        "topic": "overview",
    },
    "DFW_Strategic_Plan_2016_2020.pdf": {
        "airport": "dfw",
        "doc_type": "official",
        "source_name": "DFW_Strategic_Plan_2016_2020",
        "topic": "strategy",
    },
    "DFW-SLA.pdf": {
        "airport": "dfw",
...
def load_internal_documents():
    #multifile knowledge base logic for PDF , DOCX and TXT
...
def parse_pdf(file_content: bytes) -> list[Document]:
    "Converts PDF bytes into document objects."
    blob = Blob(data=file_content)
    documents = [doc for doc in parser.lazy_parse(blob)]
    logger.info(f"Parsed {len(documents)} pages")
    return documents

def store_documents(documents: list[Document]) -> str:
    "Adds parsed documents to the vector database."
...
def ask_question(query: str) -> dict:
    ...
    t1 = time.time()
    response = llm_chain.invoke({"input": query, "context": docs})
    t2 = time.time()
    logger.info(f"Retrieval time: {t1 - t0:.2f}s, LLM time: {t2 - t1:.2f}s")
    response = retrieval_chain.invoke({"input": query})
    t1 = time.time()
    logger.info(f"Retrieval+LLM time: {t1 - t0:.2f}s")
